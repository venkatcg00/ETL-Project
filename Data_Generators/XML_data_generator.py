...
def upload_record_to_API(api_link, xml_data):
    ...
    # Send the XML data to the API
    response = requests.post(api_link, data=xml_string, headers={"Content-Type": "application/xml"})
...
def generate_random_record(
    ...
    ET.SubElement(record, "TYPE_OF_RESOLUTION").text = random.choice(["SELF-HELP OPTION", "SUPPORT TEAM INTERVENTION"])
...
def generate_and_update_records(
    ...
    for i in range(num_records):
        ...
        if random.random() < 0.1:
            key_to_nullify = random.choice(["CONTACT_REGARDING", "AGENT_CODE", "DATE_OF_INTERACTION", "STATUS_OF_INTERACTION", "TYPE_OF_INTERACTION", "CUSTOMER_TYPE", "CONTACT_DURATION", "AFTER_CONTACT_WORK_TIME", "INCIDENT_STATUS", "FIRST_CONTACT_SOLVE", "SUPPORT_RATING"])
...
def main() -> None:
    ...
    generate_and_update_records(support_categories, agent_pseudo_names, customer_types, max_record_id, num_records, api_link_add)
