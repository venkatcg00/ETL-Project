...
def fetch_allowed_values(session: sessionmaker, 
                         table_name: str, 
                         source_name: str, 
    ...
    """
    with session() as s:
        query = text(f"SELECT DISTINCT {column_name} FROM {table_name} WHERE SOURCE_ID = (SELECT SOURCE_ID FROM CSD_SOURCES WHERE UPPER(SOURCE_NAME) = UPPER({source_name})) AND ACTIVE_FLAG = 1")
