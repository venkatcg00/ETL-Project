...
def get_latest_csv_file(folder_path: str) -> str:
    ...
    latest_file = max([os.path.join(folder_path, f) for f in csv_files], key = os.path.getmtime)
    return latest_file
...
def main() -> None:
    ...
    agent_pseudo_names: List[str] = fetch_allowed_values(sessionmaker, "CSD_AGENTS", "'AT&T'", "PSEUDO_CODE")
    customer_types: List[str] = fetch_allowed_values(sessionmaker, "CSD_CUSTOMER_TYPES", "'AT&T'", "CUSTOMER_TYPE_NAME")
    ...
    parser.add_argument('input', nargs='?', default=None, help='An optional input value')
